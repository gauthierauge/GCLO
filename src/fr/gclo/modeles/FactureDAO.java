...
//    			     "WHERE IDFacture      =  " + Facture.getIDFacture(); 
	...
	public Facture read(String ID) {
		...
		Facture Facture = null;
	     ...
	     ResultSet rs =  DataBaseInstance.executeQuery("SELECT * FROM Facture__tbl WHERE CodeFacture = '" + ID + "'");
	     try {
		     if ( rs.next() ) 
    ...
    public static String getSQLFicheTiers(String ID) {
		...
		return "SELECT CodeFacture AS NUMERO, LibelleFacture AS LIBELLE, DateFacture AS DATE, Raisonsocialetiers AS CLIENT, LibelleEtat AS STATUT, VilleTiers AS VILLE FROM Factureetatobjet WHERE CodeTiers = '" + ID + "'";
	}
    
    public static String getSQL() {
    	
		return "SELECT CodeFacture AS NUMERO, LibelleFacture AS LIBELLE, DateFacture AS DATE, Raisonsocialetiers AS CLIENT, LibelleEtat AS STATUT, VilleTiers AS VILLE FROM Factureetatobjet";
	}
